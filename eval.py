...
def Run_video(dataset,video, num_frames, num_objects,model,Mem_every=None, Mem_number=None):
    ...
    for t in range(1,num_frames):
        ...
        # memorize
        with torch.no_grad():
            prev_key, prev_value = model(F_last[:,:,0], E_last[:,:,0], torch.tensor([num_objects]))  # 1 11 512 1 37 51 F:1 3 592 813 E: 1 11 592 813
        ...
        if t-1 == 0: # 
            ...
            this_keys = torch.cat([keys, prev_key], dim=3)# 1 11 128 1 38 51   1 11 128 2 38 51
            this_values = torch.cat([values, prev_value], dim=3)# 1 11 512 1 38 51  1 11 512 2 38 51
        

        F_,M_ = dataset.load_single_image(video,t)#3 1 592 831  11 1 592 831
        #print(F_.shape)


        F_ = F_.unsqueeze(0)# 1 3 1 592 831
        M_ = M_.unsqueeze(0)# 1 11 1 592 831
        all_Ms.append(M_.cpu().numpy())
        del M_
        # segment
        with torch.no_grad():
            logit = model(F_[:,:,0], torch.tensor([num_objects]),E_last[:,:,0], this_keys, this_values )
        E = F.softmax(logit, dim=1)
        del prev_key,prev_value
        del logit
        # update
        if t-1 in to_memorize:
            keys, values = this_keys, this_values#1 11 128 6 37 51    1 11 128 5 37 51
...
def evaluate_semisupervised(all_gt_masks, all_res_masks, all_void_masks, metric):
    ...
    for ii in range(all_gt_masks.shape[0]):
        if 'J' in metric:
            j_metrics_res[ii, :],dice,iou,miou,precision,recall = db_eval_iou(all_gt_masks[ii, ...], all_res_masks[ii, ...], all_void_masks)
            sdice = sdice+dice
            siou = siou +iou
            smiou = miou+smiou
            print('sdice',sdice)
            ...
            print('recall',recall)
            print('Jaccard',j_metrics_res[ii, :])
        if 'F' in metric:
            f_metrics_res[ii, :] = db_eval_boundary(all_gt_masks[ii, ...], all_res_masks[ii, ...], all_void_masks)
    mdice = sdice/all_gt_masks.shape[0]
    iiou = siou/all_gt_masks.shape[0]
    mmiou = smiou/all_gt_masks.shape[0]
    return j_metrics_res, f_metrics_res,mdice,iiou,mmiou
...
def evaluate(model,Testloader,metric):
    ...
    for V in tqdm.tqdm(Testloader):
        ...
        pred,Ms = Run_video(Testloader, seq_name, num_frames, num_objects,model,Mem_every=8, Mem_number=None)#pred 48 592 813 Ms 1 11 47 592 813
        print(Ms.shape)
        for j in range(num_frames):
            a = pred[j,:,:]
            save_path = '/DATA/zhuzigang/davis_weights/z00/'+str(seq_name)+'/'+str(j)+'.png'
            cv2.imwrite(save_path, a * 255)
        all_res_masks = np.zeros((num_objects,pred.shape[0],pred.shape[1],pred.shape[2]))
        for i in range(1,num_objects+1):
            all_res_masks[i-1,:,:,:] = (pred == i).astype(np.uint8)# 1 24 292 813
        all_res_masks = all_res_masks[:, 1:-1, :, :]#1 46 592 813
        
        all_gt_masks = Ms[0][1:1+num_objects]# 1 47 592 813
        
        all_gt_masks = all_gt_masks[:, :-1, :, :]# 1 46 592 813
        j_metrics_res, f_metrics_res,mdice,iiou,mmiou = evaluate_semisupervised(all_gt_masks, all_res_masks, None, metric)
        for ii in range(all_gt_masks.shape[0]):
            if 'J' in metric:
    ...
    g_res = np.array([final_mean, np.mean(J["M"]), np.mean(J["R"]), np.mean(J["D"]), np.mean(F["M"]), np.mean(F["R"]),
                      np.mean(F["D"]),np.mean(mdice),np.mean(iiou),np.mean(mmiou)])
    return g_res
...
if __name__ == "__main__":
    ...
    def get_arguments():
        ...
        parser.add_argument("-g", type=str, help="2; 0,1; 0,3; etc", required=True)
